...
from .app import ApiVoidService
...
if len(sys.argv) != 2:
    print("Usage: dxlapivoidservice <configuration files directory>")
    sys.exit(1)
...
logging_config_path = os.path.join(config_dir, ApiVoidService.LOGGING_CONFIG_FILE)
...
with ApiVoidService(sys.argv[1]) as app:
    try:
        # Run the application
        app.run()
        running = True
